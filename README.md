...
**Recruiter + vault.** A secure exchange for cleared and government-adjacent hiring. Recruiters ask for what they need (clearance level, ID, expiry, right to work); job seekers provide it through a secure, expiring, audited channel — and keep those details in a personal wallet to reuse across roles. Nothing sensitive travels as an email attachment.

## What's here (Release 1)

- **Organisations, users, and RBAC** — Owner / Admin / Recruiter / Reviewer / Compliance roles, enforced in the data layer (tenant + ownership checks on every read), mirroring the PRD permission matrix (`src/lib/rbac.ts`, `src/lib/guards.ts`).
...
- **Security core** — envelope encryption (per-record DEKs wrapped by AWS KMS in ap-southeast-2, local KEK for dev), field-level encryption for PII, private S3 with SSE-KMS.
...
Next.js (App Router) · TypeScript · Tailwind · BetterAuth · PostgreSQL + Drizzle · Inngest · Resend · AWS S3/KMS/SNS · ClamAV · Docker Compose.
...
`docker compose up` builds the app and runs PostgreSQL, ClamAV, and MinIO (S3-compatible) for fully sovereign deployments — the build intended for government and sovereignty-sensitive clients. Point the AWS env vars at real S3/KMS (ap-southeast-2) for the SaaS tier.
...
- AU data residency by default (ap-southeast-2 for data and keys).
- Recruvault does **not** verify identity documents in this release; it stores and presents them for recruiter review. Verification is a later integration.
- Confirm AGSVA clearance nomenclature before each release and update `REFERENCE_SEED` in `src/lib/fields.ts` — the framework is mid-transition (PV phasing out in favour of TS-PA).

## Layout

```
src/
  db/            schema (PRD §12), connection, seed
